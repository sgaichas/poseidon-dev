...
This produces output. Visualize:
...
So the question now is, do we run `create_survey` on the output of this, or do we `create_survey` on all three truth components first, then run `calc_age2length` to get a survey sample? This should work and will eliminate the unsampled areas/times/species.
