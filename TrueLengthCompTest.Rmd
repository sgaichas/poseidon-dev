...
So, test 1 is running `calc_age2length` with the full true dataset. This should work because there will be no aggregation of the output (no, don't do this with the full dataset! example with one species instead):

```{r calcage2length-truth, warning=FALSE, message=FALSE}
...
This produced output for a single species by mistake. It ran for hours on the full truth dataset and did not complete (I stopped it). Its doing it for all species, which is a problem. Rewrote for just one species to see full length comp by area and depth layer.
...
Try again with survey sampling for nums, structn and resn, which limits the species list to fish and sharks, and also aggregates over layer and polygon. Test this:
...
length_censussurv <- calc_age2length(structn = survey_teststructnall,
...
And then `sample_fish` from `calc_age2length` output? But also we need to run `sample_fish` to get the cohort (age) output (or do we? see below) so the multinomial in there will make different distributions for smaller sample sizes. Since we are trying to represent biological sampling as one set of sampled fish which we length, then take a subset of ages and weights, this may not work. 

Or, we can sum the length atoutput from calc_age2length over size bins and back in to agecl bins to get age classes, then subsample this for ages? We also have mulen and muweight for each size bin (or is it agecl bin?) so can we back the individual weights for the aged fish subsample out of this?
