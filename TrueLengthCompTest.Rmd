...
So, test 1 is running `calc_age2length` with the full true dataset. This should work because there will be no aggregation of the output:
...
This produces output, after a looooooong time. Its doing it for all species, which is a problem. Visualize a tiny bit:
...
atf_length_census <- length_census$natlength %>%
  filter(species %in% ("Arrowtooth_flounder"))
...
It should save time to run the census survey to get true length comps, because at least it aggregates over polygon and layer. Test this:
