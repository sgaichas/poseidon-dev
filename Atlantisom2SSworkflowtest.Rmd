...
# read Atlantis output files
  atBtxt2 <- read.table(here(d.name, paste0("output", scenario.name, "BiomIndx.txt")), header=T)
...
Then we calculate lengths with `calc_age2length`
...
Now we can test the full `sample_survey_biomass` from nums to biomass index:

```{r compare-surveys}
# apply the proper weight at age (from saved census calc_age2length) to N survey

wtage <- length_census$muweight %>%
