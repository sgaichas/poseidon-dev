...
```{r, code = readLines("./config/omdimenstions.R"), eval=F}
```
...
library(ggforce)
NOBAom_redcodcap_comp$survObsFullWtAtAge[[1]][[2]] + facet_wrap_paginate(~species, ncol=1, nrow = 1, page=3, scales = "free")
...
# dataframe portion
intwtCod <- NOBAom_redcodcap_comp$survObsFullWtAtAge[[1]][[1]] %>%
  filter(species == "North_atl_cod")

intwtRed <- NOBAom_redcodcap_comp$survObsFullWtAtAge[[1]][[1]] %>%
  filter(species == "Redfish")

intwtCap <- NOBAom_redcodcap_comp$survObsFullWtAtAge[[1]][[1]] %>%
  filter(species == "Capelin") #empty

wtageCod <- NOBAom_redcodcap_comp$survObsWtAtAge[[1]] %>%
  filter(species == "North_atl_cod")

wtageRed <- NOBAom_redcodcap_comp$survObsWtAtAge[[1]] %>%
  filter(species == "Redfish")

wtageCap <- NOBAom_redcodcap_comp$survObsWtAtAge[[1]] %>%
...
wtagecl <- wtageCap
  ...
  wtagep <- ggplot(wtagecl, aes(agecl, atoutput)) +
    geom_point() +
    geom_line(aes(colour = factor(time))) + 
    scale_x_continuous(minor_breaks = c(0:max(agecl))) +
    theme_tufte() +
    theme(legend.position = "none",
          panel.grid.minor.x = element_line(colour = "grey50"),
          panel.grid.major.x = element_line(colour = "grey50"))
