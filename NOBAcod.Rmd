...
fishdiscage is empty, make sure there is no discard output from NOBA for these species.

Add a check for multiple fisheries per species, if only one then fleet irrelevant.

NOBA has multiple fleets for cod, so add fleet column to output.

Update: there is now a fleet column in output but the function is still incomplete; will not work if more than one fleet per species. 
