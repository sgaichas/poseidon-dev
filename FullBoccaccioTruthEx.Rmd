...
trueagesall <- calc_stage2age(nums_data = truth$nums,
                                 biolprm = biol, 
                                 yoy = YOY,
                                 fgs = funct.groups, 
                                 runprm = runpar)
...
**NOPE! Giving constant output or none. Back to the drawing board on true ages.** 

See new file TestCalcZ.Rmd testing `calc_Z` separately and then I will do another separate one testing `calc_stage2age`. Once they work I will update this.
...
# run survey  and sample fish on this for completeness? yes to aggregate
survey_trueNage_ss <- create_survey(dat = trueNatage_ss,
                                 time = timeall,
                                 species = survspp,
                                 boxes = boxall,
                                 effic = effic1,
...
trueNagesamp_ss <- sample_fish(survey_trueNage_ss, effNhigh)
