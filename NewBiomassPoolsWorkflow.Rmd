...
In `om_species` need to check for biomass pools output and append that to the `truebio_ss` object? If one object can use existing `om_index` on bio pools.
...
If a single true bio object is created, may have to modify selectivty definition for biomass pools in `om_index`
