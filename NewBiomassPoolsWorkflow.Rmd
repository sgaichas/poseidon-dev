...
```
So we want "N" nitrogen output. In theory this can be achieved with `atlantisom::load_nc`with `select_variable = "N"`. Looks like it works, output is N for our selected biomass pool species over all polygons, layers, and times.
...
I don't think we have a function that aggregates over layets and converts N to biomass but there are pieces in other functions.
