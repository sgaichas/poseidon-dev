...
#fish_timesteps <- seq(fish_times[1], max(fish_times), by=fstepperyr) #last timestep
...
wtage1 <- data.frame(species=rep(species_ss, each=10),
                    ...
                    wtAtAge=rep(1.0,length(species_ss)*10))
...
Do some comparisons of biomass surveys with truth. Something is going way wrong in the calculation of survey biomass based on numbers times weight at age, looks like division by 1000 could fix it?.
...
plotB <-ggplot() +
  geom_line(data=trueB_ss, aes(x=time/stepperyr,y=atoutput, color="survey census B"), 
            alpha = 10/10) +
  geom_point(data=survObsBiom, aes(x=time/stepperyr,y=atoutput/1000, color="survey sample B from nums * wtAtAge"), 
            alpha = 10/10) +
  theme_tufte() +
  theme(legend.position = "top") +
  labs(colour=scenario.name)
