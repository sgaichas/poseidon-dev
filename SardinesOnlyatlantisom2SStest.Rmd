...
Do true catch numbers times weight at age match catch in biomass? No. It is about 16 times too high relative to catch.txt output, and shifted one year back (which is due to the floor function, and an easy fix). 
...
plotcatch <- ggplot() +
  geom_line(data=truecatchbio_ss, aes(x=time/365,y=atoutput, color="true catch bio from txt"),
            alpha = 10/10) +
  geom_point(data=catchbioN_ss, aes(x=yr,y=anncatchB/16, color="true catch bio from N/16"),
            alpha = 10/10) +
  theme_tufte() +
  theme(legend.position = "top") +
  labs(colour=scenario.name)
...
Check this against true total catch numbers above. Should be the same.
...
#Test writing CPUE in biomass
  stocksynthesis.data <- SS_write_ts(ss_data_list = stocksynthesis.data,
                ts_data = list(survObsBiom$atoutput[fish_years],
                  catch_total$catch[catch_total$yr %in% fish_years]),
                CVs = c(CVs$survey,
                        CVs$fishery),
...
# below fails for fishery data if I keep timestep aligned with year 31
# Error in `[.data.frame`(comp_matrix[[i]], , as.character(bins[[i]])) : undefined columns selected

# assigning fishery comps to year 30 start (remove +1 after floors above) 
# I get a different error
# Error in `[<-.data.frame`(`*tmp*`, indices, "Seas", value = c(1, 1, 1, : replacement has 49 rows, data has 2045
