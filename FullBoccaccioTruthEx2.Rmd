...
After corrections were applied to `calc_Z` we were closer to the ballpark in NOBA. While the estimated true ages no longer just drop to 0, the patterns here don't seem like a good enough approximation of how age class would decay. The numbers at true age for NOBA cod are closer because there are only 2 ages within a stage to split, but they still [don't match atlantis output where we have it for NOBA](https://sgaichas.github.io/poseidon-dev/TestCalcZ.html), so I will do further testing with this `calc_Z` and `calc_stage2age`. 
...
We may need to rethink the annual Z we are estimating to do the approximation.
...
This is true catch at (st)age for bocaccio (in age classes, not true ages):
...
```{r cattrueage, warning=FALSE, message=FALSE}
...
These still don't look quite right.

Length sample with user specified max length bin (200 cm):

```{r userset-maxlen, echo=TRUE, eval=FALSE}
...
saveRDS(catch_length_census_ss, file.path(d.name, paste0(scenario.name, "catchlength_census_sardhakeboca.rds")))
