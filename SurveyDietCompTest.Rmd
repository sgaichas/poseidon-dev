...
The current partial `sample_diet()` function needs to change because it expects a wide format file of diet proportions, rather than a long-format file of consumption in tons which is the output of `create_survey_diet()`. 
...
The current function removes predators not expected on a fish trawl survey, removes prey types that are normally not counted, then adds uniform error to half the remaining diet proportions, removes a random subet of prey accounting for <0.25 diet proportion, then renormalizes. I would rather have a function that could be used on any "survey" so have already removed hardcoded predator selection. I think also the distributional assumptions need revision; a Dirichlet seems most appropriate to simulate continuous proportions. 
...
sample_diet <- function(dat, fgs) {
  ...
  # read in dat, long format output of load_diet_comp, or create_survey_diet, both have layers aggregated
  
  # load_diet_comp has no polygon information and is proportion, create_survey_diet has polygons and is consumption in tons
  
  # first aggregate to global diet comp in proportion
  
  # then apply aggregation bias
