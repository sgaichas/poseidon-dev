...
samp4 <- sample_diet(survey_cons, fgs, unidprey = 0.3, alphamult = 10000)
...
Here we compare survey diet comp with no bias and variance, with some bias (unidprey = 0.3 or up to 30% of each prey proportion could be set to unidentified) and extremely low to moderate Dirichlet observation error (alphamult = 10000000 and alphamult = 10000):
...
for(i in 1:length(preds)) {
  cat("  \n####",  as.character(preds[i]),"  \n")
  print(plotdiet(dat = filter(samp3, species %in% preds[i]), namedat = "Biased survey diet", compdat = filter(samp4, species %in% preds[i]), namecomp = "Bias and variance survey diet")) 
  cat("  \n")
}
```
